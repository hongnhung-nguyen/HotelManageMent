...
namespace src.DB
{
    class RoomDB
    {
        ...
        public static string GetMaKHByMaPhong(string MaPhong)
        {
            ...
            string query = $"select MAKH from HOTEL_PUBLIC.A_PHIEUDATPHONG A WHERE A.MAPHONG = '{MaPhong}'";
            OracleCommand command = new OracleCommand(query, conn);
        ...
        public static string GetLoaiPhongByMaPhong(string MaPhong)
        {
            ...
            string query = $"select LOAIPHONG from HOTEL_PUBLIC.A_PHONG A WHERE A.MAPHONG = '{MaPhong}'";
            OracleCommand command = new OracleCommand(query, conn);

            var reader = command.ExecuteReader();
            while (reader.Read())
            {
                return (reader["LOAIPHONG"].ToString());
            }
            conn.Close();
            return "";
